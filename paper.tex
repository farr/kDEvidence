...
\eads{\mailto{w.farr@bham.ac.uk}, \mailto{ilyamandel@chgk.info} \mailto{dan.stevens@u.northwestern.edu}}
...
adaptive than the boxes of Ref.~\cite{Littenberg2009}, and more
...
dimension, so for reasonable numbers of points a higher-dimensional kD
tree will have fewer subdivisions along each dimension.  Second, as
the dimensionality increases the volume ``at the corners'' of each kD
...
To address the arbitrary orientation of points, the modified jump
...
a threshold, $\Nbox$.  We then find the principal axes of the points
...
within the rectangular box. The new point is chosen uniformly from
...
the volume of the principal-component box containing $\vtheta_{i+1}$.
...
of the parameter space.  Because this introduces a dependence on the
previous states of the MCMC, the jump proposal is no longer
Markovian.  However, it remains \emph{asymptotically} Markovian, which
...
amount of work per tree level, and there are $\log N$ tree levels.
...
\cite{Raymond2009,Raymond:2010,Raymond2012}.
\texttt{LALInferenceMCMC} is a MCMC code, based on the LIGO algorithms
library, designed to sample the posterior on parameters of
compact-object binaries (masses, orbital orientation, distance, sky
position, etc) implied by their gravitational wave signals in
...
described a new technique that allows for efficient evidence
...
and then using this interpolated PDF to propose efficient inter-model
jumps.
...
X-ray binaries, as described in Ref.~\cite{Farr2010}.  In addition to
model comparison, the PDF interpolation described here can be useful
in multi-step MCMCs to improve the sampling efficiency by selecting
...
support from a Northwestern University Summer Undergraduate Research Grant.
