...
%, the ratio of Bayesian probabilities for the two models given the observation data.
...
%\ilya{Something about how nested sampling distributes points according to the prior, not the posterior, and therefore can miss modes that are disfavored by prior.  Also, it does not exploit any information about the posterior to make the evidence calculation more efficient.}

Markov Chain Monte Carlo (MCMC) methods attempt to
stochastically produce parameter samples with density proportional to
the posterior probability distribution.   In MCMC techniques, the primary target is an accurate estimate of the posterior distribution.  (We note that an alternative stochastic method for exploring a model parameter space, nested sampling \cite{Skilling:2004, Skilling:2006,Feroz:2009}, focuses on evidence computation rather than sampling the posterior probability density functions.)  \will{Will, are you happy with nested sampling in a parenthetical expression, without further comments?}
...
from MCMC samples is to compute the harmonic-mean estimator, but
this estimator of the evidence suffers from infinite
variance\cite{NewtonRaftery:1994,Chib:1995,vanHaasteren:2009}.  Various
methods have been proposed to stabilize the harmonic-mean estimator \will{Citation}, but all
require some assumptions about the shape of the posterior distribution
that may not be satisfied in practice.  MCMC implementations with parallel tempering \cite{EarlDeem:2005} allow for evidence computation via thermodynamic integration, but these can be computationally costly.  



Reference \cite{Weinberg2009} gives a method for directly computing
the evidence integral from existing MCMC samples by using a kD-tree data structure to decompose a
...
models, the information about the choices of parameter values in the
...
Reference \cite{Littenberg2009} proposed an alternate method for producing
inter-model jumps in an RJMCMC that relies on a box decomposition of
parameter space, but uses fixed-sized boxes.  The box size is
fixed, so the method cannot adapt to the local structure of the
posterior, and becomes asymptotically inefficient for high-dimensional parameter spaces or highly peaked posteriors.  Meanwhile, the approximation to the
...
Reference \cite{Ascasibar2005}.  The kD-tree interpolation is effective
in parameter spaces of arbitrary dimensionality, and is quite space-efficient,
...
MCMC samples used to construct the interpolation.  
...
sampling \cite{Skilling:2004,Skilling:2006} and its variant MultiNest \cite{Feroz:2009}).  Although nested sampling can be used to compute the posterior PDFs within each model along
with the evidences for the various models, perhaps the most common
technique for computing posterior PDFs in the context of a model is
...
techniques are promising, but in some cases suffer \cite{Farr2010} \will{Is this really the reference you meant?}
...
%The solution to this problem was to add a uniform distribution to the proposal PDF, in the form of one ``fictitious'' point per brick.  The extra uniform distribution from the fictitious points can represent a significant cost, since inter-model jumps proposed from the uniform distribution are unlikely to be accepted.  
However, the bricks in this
algorithm do not adapt to the local structure of the PDF.  One must
either use small bricks to capture the local structure of the PDF,
placing many bricks in regions without MCMC samples (which can increase memory management and access costs, even for sparse matrices), or use large
bricks, missing the local structure of the PDF in exchange for fewer
empty bricks. \will{Are you happy with this bit of editing?}
...
We obtain $10^4$ individual model MCMC samples within each model, and
use the kD-tree interpolation method described above to propose
...
containing the desired number of points).  For each truncation, we
performed an RJMCMC with the resulting interpolated jump proposal.
The acceptance rate versus truncation number appear in Figure
\ref{fig:acceptRate}.  The more points in each box of the tree when
...
The relative error on the determination of the evidence ratio scales
...
rate of inter-model jumps significantly, the interpolation method
described above can dramatically improve the runtime of an RJMCMC.
