...
parameter space, representing the ``total probability'' in that model.
Models with larger evidence are preferred; the ratio of the evidences
of two models is the Bayes ratio between them.  The product of the
Bayes ratio and the ratio of prior probabilities for the model gives
the odds ratio for the models, the ratio of Bayesian probabilities for
the two models given the observation data.
...
practical.  Nested sampling \cite{Skilling:2006,Feroz:2009} \ilya{More
  references?} is a stochastic method for exploring the parameter
space of a model while computing the evidence integral statistically
\ilya{I want a better word than ``statistically'' to indicate that
  it's doing something akin to Monte-Carlo integration (but I would
  like to reserve Monte-Carlo for MCMC).}.  

\ilya{Something about how nested sampling distributes points according
  to the prior, not the posterior, and therefore can miss modes that
  are disfavored by prior.  Also, it does not exploit any information
  about the posterior to make the evidence calculation more
  efficient.}

In contrast to nested sampling, where computation of evidences is the
primary objective, Markov Chain Monte Carlo (MCMC) methods attempt to
stochastically produce parameter samples with density proportional to
the posterior probability distribution.  In MCMC techniques, the
primary target is an accurate estimate of the posterior distribution.
It is not straightforward to compute the model evidence from the MCMC
samples.  The most direct way to estimate the evidence for a model
from MCMC samples is to compute the harmonic-mean estimatator, but
this estimator of the evidence suffers from infinite
variance\cite{NewtonRaftery:1994,Chib:1995,vanHaasteren:2009}; various
methods have been proposed to stabilize it \will{Citation}, but all
require some assumptions about the shape of the posterior distribution
that may not be satisfied in practice.

Reference \cite{Weinberg2009} gives a method for directly computing
the evidence integral using a kD-tree datastructure to decompose a
parameter space into boxes containing the MCMC sample points.  The
integral is approximated as a sum over box volumes.  This method is
promising, but it is not clear in general what statistical and
systematic errors it introduces and how these are affected by the
shape of the posterior distribution from which the MCMC samples.
...
available to the algorithm.  Each time an RJMCMC transitions between
models, the information about the good parameter values in the
previous model is lost; subsequent jumps into that model must ``start
fresh,'' and are correspondingly unlikely to be accepted, delaying
convergence of the RJMCMC sample chain.  

Here we demonstrate a technique for improving the acceptance ratio of
...
datastructure to construct an approximation to each model's posterior
parameter distribution.  We propose jumps into each model drawn from
the approximation to the posterior.  Because jumps are proposed
...
models, and convergence is generally rapid.  The approximation to the
posterior distribution produced by the kD-tree is a constant-in-box
...
Refernce \cite{Ascasibar2005}.  The kD-tree interpolation is effective
in arbitrary numbers of dimension, and is quite space-efficient,
requiring $\order{N \log N}$ storage space and $\order{\log N}$ time
to produce each proposed jump, where $N$ is the number of single-model
MCMC samples used to construct the interpolation.  Reference
\cite{Littenberg2009} proposed an alternate method for producing
inter-model jumps in an RJMCMC that relies on a box decomposition of
parameter space, but uses fixed-sized boxes; because the box size is
fixed, the method cannot adapt to the local structure of the
posterior, and therefore becomes asymptotically less efficient than
the adaptive kD-tree decomposition in either acceptance ratio or
storage space as the parameter-space dimension increases or the
posterior becomes more peaked.  \ilya{Mention something (with
  reference) about the 10-way RJMCMC in the black-hole mass paper?}
...
introduce the kD-tree datastructure used to decompose parameter space
into boxes for the interpolation.  In Section \ref{sec:efficiency} we
...
examples of some other uses of the interpolated jump proposal in a
single-model MCMC context.  Finally, in Section \ref{sec:conclusion}
...
$p(\vtheta^{(j)})=p(\vtheta|d,M)$ , then the next state $(j+1)$ must
be sampled from the PDF as well, so that
$p(\vtheta^{(j+1)})=p(\vtheta|d,M)$; this condition is known as
``detailed balance.''

One way to produce such a sequence of samples is via the
Metropolis-Hastings algorithm, first proposed in Reference
\cite{Metropolis:1953}, and later generalized by Hastings
\ilya{Citation?}:
\begin{enumerate}
...
on the past history.  It must also allow any state within the allowed
prior volume to be reachable by the MCMC.  Any jump proposal that
satisfies these properties is suitable for an MCMC.

The jump proposal is the most important choice in the MCMC, as it
determines the sampling efficiency of the algorithm, i.e., the
necessary length of the chain before it converges to the posterior
PDF.  Creating an efficient jump proposal distribution requires an
understanding of the structure of the parameter space which may not be
available until the PDFs are found, creating a Catch-22; one
possibility for resolving this infinite loop is described in Section
\ref{sec:examples}.
...
from large variance and bias as well.  An alternative approach to
model selection among a set of models is based on performing an MCMC
in a ``super-model'' that encompasses all of the models under
...
marginalizing over the remaining parameters.  If we assume uniform
priors on the models, the evidence for a given model is proportional
...
distributed according to the posterior PDF.  In other words, the size
of a neighborhood is inversely proportional to the local point
density.  The proposed jumps are drawn from a piecewise-constant
(i.e.\ constant on each neighborhood) interpolation of the PDF.  There
are various techniques that could be used to construct the set of
neighborhoods associated with each point.
...
cost for computing the Voronoi regions increases rapidly with
dimension.  % Apparently the exact computational cost is not known in
            % arbitrary dimension.

We propose to use a decomposition of parameter space into
neighborhoods based on a datastructure called a kD-tree (see, e.g.\
\cite{Berg2008} or \cite{Gaede1998}).  The decomposition is more
adaptive than the cells of Ref.~\cite{Littenberg2009}, and more
efficient in high dimension than the Voronoi decomposition.
...
median-finding algorithm.  Tree construction thus costs $\order{N \log
  N}$ in time, and the tree consumes $\order{N \log N}$ space.  As an
