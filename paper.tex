...
distribution from the fictituous points can represent a significant
cost, since inter-model jumps proposed from the uniform distribution
are unlikely to be accepted.  Additionally, the bricks in this
...
placing many bricks in regions without MCMC samples, or use large
bricks, missing the local strucutre of the PDF in exchange for fewer
empty bricks.
...
Here we propose to use a decomposition of parameter space into
...
Gaussian distributed with unkonwn mean, $\mu$, and standard deviation,
$\sigma$
%
\be
...
volume.  The Cauchy distribution is so much broader than the Gaussian
(it has no finite moments), that with uniform model proirs, the
